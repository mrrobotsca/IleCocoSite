...
const formatDate = (iso: string) => new Date(iso).toLocaleString('en-CA', { dateStyle: 'short', timeStyle: 'short' })
            ...
            <tbody className="divide-y divide-zinc-100">
              {rows.map((r) => (
                <tr key={r.id} className={r.unsubscribedAt ? 'opacity-50' : ''}>
                  <td className="whitespace-nowrap px-3 py-2.5 text-zinc-600">{formatDate(r.createdAt)}</td>
                  ...
                  <td className="px-3 py-2.5">
                    {r.unsubscribedAt ? (
                      <span className="rounded-full bg-zinc-200 px-2 py-0.5 text-xs text-zinc-600">unsubscribed</span>
                    ) : (
