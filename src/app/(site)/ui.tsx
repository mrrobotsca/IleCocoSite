...
const VARIANTS = {
  ...
  ghost: 'bg-transparent text-charcoal-deep border-[1.5px] border-charcoal-deep hover:bg-charcoal-deep hover:text-porcelain',
  ghostLight: 'bg-transparent text-porcelain border-[1.5px] border-porcelain/30 hover:bg-porcelain hover:text-charcoal-deep',
} as const
...
export const PillButton = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'dark', size = 'md', withArrow = false, className, children, ...rest }, ref) => (
    <button ref={ref} className={cn(PILL_BASE, VARIANTS[variant], SIZES[size], className)} {...rest}>
      {children}
...
export const Eyebrow = ({ children, className, light = false }: { children: React.ReactNode; className?: string; light?: boolean }) => (
  <span
    className={cn(
...
>(({ variant = 'dark', size = 'md', withArrow = false, className, children, onClick, ...rest }, ref) => {
