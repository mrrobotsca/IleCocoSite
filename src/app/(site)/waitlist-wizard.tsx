...
export const WaitlistWizard = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: WaitlistSubmission = WaitlistSubmissionSchema.parse({
        ...form,
        lang,
      });
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Decorative band — sticky header */}
            <div className="relative flex-shrink-0 bg-ash-green px-6 pb-5 pt-[max(env(safe-area-inset-top),1.75rem)] sm:px-9 sm:pt-7">
              <Bloom
                size={36}
              ...
              <button
                ...
                className="absolute right-4 top-[max(env(safe-area-inset-top),1rem)] grid h-9 w-9 place-items-center rounded-full bg-porcelain/80 text-charcoal-deep transition-colors hover:bg-porcelain disabled:opacity-50 sm:top-4"
              >
                <X size={18} />
              </button>
              <div className="flex items-center gap-3 pr-12">
                ...
                <div className="min-w-0">
                  <h2
                    id="wizard-title"
                    className="font-display text-[20px] font-bold leading-tight text-charcoal-deep sm:text-[24px]"
                  >
                    {c.title}
                  </h2>
                  <p className="text-[13px] text-charcoal-deep/75">
                    {c.subtitle}
                  </p>
                </div>
              </div>
              ...
              {step < TOTAL_STEPS && (
                <div className="mt-5">
                  ...
                  <p className="mt-1.5 font-display text-[11px] font-semibold uppercase tracking-[0.06em] text-charcoal-deep/60 sm:text-[12px]">
            ...
            {/* Scrollable body */}
            <div className="flex-1 overflow-y-auto overscroll-contain px-6 pt-6 sm:px-9 sm:pt-7">
              <AnimatePresence mode="wait" initial={false}>
                {step === 0 && (
                  <StepShell key="s1">
                    <Field
                      label={c.child.firstName}
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setField("childFirstName", e.target.value)
                        }
                ...
                {step === 1 && (
                  <StepShell key="s2">
                    <Field
                      label={c.preferences.location}
                      error={errors.preferredLocation}
                    >
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                        {(["somerled", "lachine", "either"] as const).map(
                          (opt) => (
                            <SelectableCard
                              key={opt}
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setField("desiredStartDate", e.target.value)
                        }
                        min={today()}
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <Field label={c.contact.email} error={errors.parentEmail}>
                        <input
                          ...
                          onChange={(e) =>
                            setField("parentEmail", e.target.value)
                          }
                      ...
                      <Field label={c.contact.phone} error={errors.parentPhone}>
                        <input
                          ...
                          onChange={(e) =>
                            setField("parentPhone", e.target.value)
                          }
                ...
                {step === TOTAL_STEPS && (
                  <motion.div
                    ...
                    className="flex flex-col items-center py-10 text-center sm:py-6"
                  >
            ...
            <div className="flex-shrink-0 border-t border-charcoal-deep/8 bg-porcelain px-6 pb-[max(env(safe-area-inset-bottom),1rem)] pt-4 sm:border-t-0 sm:px-9 sm:pb-7 sm:pt-2">
              {step < TOTAL_STEPS ? (
...
const inputCls = (hasError: boolean) =>
  cn(
    "w-full rounded-2xl border-2 px-4 py-3 font-display text-[16px] text-charcoal-deep outline-none transition-colors placeholder:text-ink-faint",
